...
def load_urls(page):
    """読み込み関数"""
    page.controls[0].details.controls = []

    if not URLS_PATH.exists():
        snackbar = ft.SnackBar(
            ft.Text(
                "URLs.htmlが見つかりませんでしたが、新たに作成することができます。"
            ),
            bgcolor=ft.colors.YELLOW_500,
        )
        create_wrapper_detail(page.controls[0], "ツール/勤怠/申請系")
    else:
        ...
        for content in urls_contents:
            if content.strip().startswith("<details "):
                if indentation_level == 0:
                    pointer = page.controls[0].add_click(None)
                    indentation_level += 1
                else:
                    pointer = pointer.add_click(None)
                    indentation_level += 1
            if content.strip().startswith("""<summary class="summary">"""):
                match = re.search(
            ...
            if content.strip().startswith("""<span class="key"""):
                ...
                if match:
                    ...
                    tmp_pointer.url.value = de_double_backslash(
                        de_sunitize(match.group(2))
                    )
                    tmp_pointer.show_text.value = de_sunitize(match.group(3))
                    tmp_pointer.drop_mune.value = match.group(1)

            if content.strip().startswith("""<a href="""):
                match = re.search(
                    r'href="(.*?)".*?>(.*?)</a>',
                    content.strip(),
                )
                if match:
                    tmp_pointer = pointer.add_url_click(None)
                    tmp_pointer.url.value = de_double_backslash(
                        de_sunitize(match.group(1))
                    )
                    tmp_pointer.show_text.value = de_sunitize(match.group(2))

            if content.strip().startswith("""<button onclick"""):
                match = re.search(
                ...
                if match:
                    ...
                    tmp_pointer.copy_body.value = de_double_backslash(
                        de_sunitize(match.group(1).replace(r"\n", "\n"))
                    )
                    tmp_pointer.show_text.value = de_sunitize(match.group(2))

            if content.strip().startswith("""<span>"""):
                match = re.search(
                    r"<span>(.*?)</span>",
                    content.strip(),
                )
                if match:
                    ...
                    tmp_pointer.text.value = de_sunitize(match.group(1))
            if content.strip().startswith("""<!--MANUAL LINE BREAK-->"""):
...
class Editor(ft.Column):
    ...
    def get_format_all(self):
        ...
        # URLS_PATH.write_text("".join(ret_string), encoding="utf-8")
        # URLS_PATH.write_text("".join(JAVASCRIPT_KEYDOWNS), encoding="utf-8")
        return ret_string
...
class UrlLink(ft.Row):
    ...
    def get_format_all(self):
        ...
        return [
            rf"""{key_string}<a href="{double_backslash(sunitize(self.url.value))}" target="_blank" rel="noopener noreferrer">{sunitize(self.show_text.value)}</a><br>"""
            + "\n"
        ]
